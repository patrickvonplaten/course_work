...
Appling the short term fourier transform converts a signal $X(t)$ to multiple $(F,T)$ binsthat can be plotted in a \emph{spectrogram}.
...
The first theoretical framework for automated source separation was introduced by A.S. Bregman in 1990 \cite{bregman1994auditory}. Systems using this approach are called \emph{computational auditory scene analysis} and have first acceptable results \cite{4429320}.

Here, we want to concentrate on the first methods using machine learning approaches to
solve the \emph{cocktail party problem}, namely \emph{Spectral Clustering} \ref{sub:spectral_clustering} and \emph{Single-channel multi-talker separation using deep learning techniques} \ref{sub:single_channel_multi_talker_separation_using_deep_learning_techniques}.
