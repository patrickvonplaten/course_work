...
\begin{algorithm}[H]
	...
	\begin{enumerate}
		\item Segmentation:
	    	...
	    	\item Grouping:
			\begin{itemize}
				\item $(T,F)$ regions are combined to streams corresponding to sound sources
...
\begin{equation} \label{eq:deep_clust_kmeans}
	\hat{Y} = \text{argmin}_{Y} || {V - Y(Y^TY)^{-1}Y^TV} ||_F^2
\end{equation}
...
Since $(Y^TY)^{-1}Y^T \in \reals^{C \times N}$ presents the means of all clusters, minimizing the 
...
\begin{equation}
	\hat{w_k} = \frac{g}{\sigma} \odot (w_k - \mu) + b 
\end{equation}
...
method as stated in \cite{TasNet} for a two-speaker single channel source separation. Both networks were trained on the WSJ0-2mix dataset. 
