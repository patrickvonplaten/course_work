...
There is one disadvantage using the deep clustering approach which is its high latency 
...
It consist of a deep long short term memory network that takes $w_k$ as input and 
...
$\sigma$ and $\mu$ present the variance respectively of the vector $w_k$ and $g,b \in 
...
network directly be trained on a signal to noise ratio which is the common 
...
\begin{equation} \label{snr_inv}
	\text{SNR}_i  = 10\log_{10}\frac{|s_{i}|^2}{e_{i}}
\end{equation}
...
SHOULD EXPLAIN HERE WHY SIGNAL TO NOISE RATIO IS USED IN TRAINING AND NOT JUST MIN NOISE.
Therefore our loss function is to minimize the inverse $SNR$ of all sources:
...
layer \ref{ssub:decoding} respectively will be learned to act ``filterbanks'', thus 
...
\begin{itemize}
	...
	signal chunk $m_{s,k}$ was output on which range of the output nodes and thus the
...
to signal with a predefined number of speakers. 
...
\[
	SDR := 10\log_{10} \frac{|s_{\text{target}}|^2}{|e_{\text{interef}} + e_{\text{noise}} + e_{\text{artif}}}
\]
...
As we can see both networks the exact same result. Deep clustering has the advantage 
...
and thus could be applied to real-time application, whereas deep clustering methods cannot. Finally it can be said, that \emph{TasNet} can be regarded to be more of an \emph{end-to-end} model, where all components belong to one autoencoder and are trained together. 
...
networks are very useful methods to when it comes to source separation. 
...
While writing this paper a new \emph{end-to-end} system using so-called \emph{Unfolded Iterative Phase Reconstruction} was presented \cite{2018arXiv180410204W} that yielded a new state-of-the-art $SDR$ of 
\textbf{12.8} on the dataset WSJ0-2mix.

There surely will be more improvement in the near future so that systems will 
be able to cleanly separate mixture speech and solve the long-lasting \emph{Cocktail 
party problem}.
