...
\begin{equation} \label{eq:advanced_cost}
	C(\theta) = |VV^T - YY^T|_F^2 = sum_{i,j}(v_i^T v_j - y_i^T y_j)^2 = sum_{i,j; y_i = y_j} (v_i^T v_j - 1)^2 +  sum_{i,j; y_i \ne y_j} (v_i^T v_j)^2 = sum_{i,j; y_i = y_j} (-2 v_i^T v_j) + 1 + sum_{i,j} (v_i^T v_j)^2  
\end{equation}
...
\begin{equation} \label{eq:advanced_cost2}
	...
	sum_{i,j; y_i = y_j} |v_i - v_j|^2 - 1 + sum_{i,j} (v_i^T v_j)^2 + sum_{i,j; y_i = y_j} |v_i - v_j|^2 + sum_{i,j} (v_i^T v_j)^2 - N  
\end{equation}
...
minimize the first addend. The second addend and third addend can be seen as a regulizer preventing trivial 
solutions.
As a conclusion, the embedding $V$ is very similar to the low rank approximation matrix used in 
spectral clustering.
To optimize computation performance the expensize $N \times N$ matrix calculation in equation \ref{advanced_cost2} can be replaced by the less expensive $K \times K$ with $K$ being the embedding dimension of V which is 
...
Analog to \emph{spectral clustering}, we use the \emph{k-means} algorithm by minimizing its inference cost with the following equation \cite{SingleChannelSourceSeparation}:

\begin{equation} \label{eq:deep_clust_kmeans}

\hat{Y} = argmin_{Y} {V - Y(Y^TY)^{-1}Y^TV}

\end{equation}

$\hat{Y}$ are used as binary masks to separate the sources. 
