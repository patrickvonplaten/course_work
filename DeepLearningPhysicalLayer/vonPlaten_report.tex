...
Thus having a code set $M$ of length $|M|=2^k$, needs $|M|$ differnt amplitude/phase 
combinations. Having defined $|a|$ distinct amplitude values $a$ and $|\phi|$ distinct phase 
values $\phi$, the equation $|\phi| \times |a| \ge m$ must hold true.
Each code symbol $m \in M$ can then be represented by a $(a,\phi)$ point in the complex,
with $a$ presenting  different unit circles and $\phi$ the angle. Figure \ref{fig:16QAM}
presents such a scheme with $m=16, |a| = 3, |\phi| = 12$.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\linewidth]{images/16QAM.png}
	\caption{Example of QAM modulation scheme: M = 16, L = 3}
	\label{fig:16QAM}
\end{figure}

It can be seen that the arrangement of the points in the grid aims at being being equally
spaced in a way that maximizes the euclidean distance between each point, given the 
constraints of $|a| = 3, |\phi| = 12$ and an assumption about the noise in the channel 
...
source symbols being the ``transmitter network'' and ``receiver network'' (see \ref{:applying_deep_learning_to_communication_systems}).
...
``reading'' the amplitude and phase from the carrier wave. Finding the exact time 
...
We will revisit the synchronization problem when when applying \emph{deep learning}
...
As stated in \cite{Goodfellow-et-al-2016}, convergence in a \emph{zero-sum game} (or minmax game) is not a local optimum in $v_{\theta^{(g)},\theta^{(d)}}$, but ``that are simultaneously minimafor both networks' costs''.
...
In section \ref{generative_adversarial_networks_based_communicaton_system}, we will explain in detail how generative adversarial networks can 
...
\begin{enumerate}
	\item The ``channel'' layer in the neural network (layer that models the channel response) 
	      often fails to be a accurate model of the ``real'' channel response. 
	...
	\item The receiver network is supposed to work directly on the quadrature samples 		of the demodulated carrier wave and thus has problems to effectively 
	      synchronize received quadrature samples and transmitted quadrature samples 		when used with ``real'' channels.
\end{enumerate}
...
Other approaches, such as a two-phase training strategy where at first both 
the transmitter and the receiver network are training on a ``channel'' layer and at 
second only the receiver is finetuned using a ``real'' channel response exist as 
well \cite{2018ISTSP..12..132D}, but will not be the focus of this report.
...
\begin{itemize}
	\item Neural network transmitter
	\item Neural network embedding of the channel
	\item Neural network receiver
\end{itemize}
...
The neural network transmitter presents the encoding function $f_{e, \theta_e}: v \to x, x \in \mathbb{R}^n (\text{ or } \mathbb{C}^{\frac{n}{2}})$.
In the context of a communication systems, we have $n$ different channels we can use 
to send the symbol $s_i$ presented by $v_i$. Every combination of the $n$ entries in $x=\left[x_1,x_2,...,x_n\right]$ is a data point in the modulation plane (compare to \ref{sub:structure_of_the_communication_system} and \ref{fig:16QAM}). Since in conventional 
modulation techniques these ``modulation data points'' are complex values we can 
also consider the first half of the output layer $x$ of the neural network transmitter to be real valued and the second half to be complex valued. Due to reasons of simplicity, 
...
$m$ different input vectors, there are also $m$ different encoded vectors $\left[x_1,...,x_m\right]$ being the modulated representations of the symbols $s_1,...,s_m$.

Let's take for example the case of $n=2$. That would mean that there are two channels to 
send data. The case of having $n=2$ channels is equivalent to quadrature modulatino where
the first and second channel would represent the $(x,y)$ data points that can be converted
to the $(a,\phi)$ data points of amplitude and phase via $a = \sqrt{x^2+y^2}$ and $\phi = \tan(\frac{y}{x})$ as can be seen in \ref{fig:16QAM}.

The neural network transmitter is made up of the input layer $v$ havig $m$ nodes, multiple hidden 
...
$m$ or less nodes \cite{2018ISTSP..12..132D}, \cite{2018arXiv180506350O}, \cite{DBLP:journals/cor/OSheaH17}.
...
to modulation data points $x_i$ as can be seen in \ref{fig:TODO}.
...
``real'' noise of and is the most probable reason why autoencoder communication 
system fail to show the same results when used in practise. 
...
$s: \hat{v}_i\to \frac{e^{v_i}}{\sum_{j=1}^{n} v_j}$ is applied to every output 
...
individually for every single parameter is $\theta_e$ and $\theta_d$ by calculing 
the respective gradients. Having $|theta_e| + |theta_d|$ trainable parameters 
means that we also need to calculate $|theta_e| + |theta_d|$ different gradients 
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.32\linewidth]{images/learnedModulation.png}
...
will be modulated into a carrier wave at sent of the ``real'' channel. 
...
the symbols $s_k$. 
Having learned a new encoding scheme, the ``real'' channel response has changed at well, 
so that the $h_{1,\theta_h}$ needs to be relearned and so on...
This training procedure will eventually lead to smaller and smaller changes and converge.
...
did not cope with the problem of unsynchronized electrical oscilators needed to 
send the outputs of the transmitter neural network to the receiver neural network.
